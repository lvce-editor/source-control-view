...
test('requests file and folder icons directly from the icon theme worker', async (): Promise<void> => {
  ...
  const requests = [
    { name: 'index.ts', path: '/test/index.ts', type: 1 },
    { name: 'src', path: '/test/src', type: 2 },
  ]
...
test('propagates icon theme worker errors', async (): Promise<void> => {
  ...
  await expect(requestFileIcons([{ name: 'index.ts', path: '/test/index.ts', type: 1 }])).rejects.toThrow('Icon theme unavailable')
