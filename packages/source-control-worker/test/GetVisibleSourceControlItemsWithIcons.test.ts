...
test('getVisibleSourceControlItemsWithIcons - preserves selection when scrolling away and back', async (): Promise<void> => {
  ...
  const second = await getVisibleSourceControlItemsWithIcons(items, 20, 40, {}, first.fileIconCache, selectedItem)
  expect(second.visibleItems.some((item) => item.selected)).toBe(false)
